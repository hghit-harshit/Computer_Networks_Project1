...
void print_leaderboard(const std::string& raw_json)
{
    ...
    std::priority_queue<std::pair<int,std::string>> leaderboard;
    for(auto standing : leaderboard_json["standings"])
    {
        int score = standing.value("score", 0);  // default to 0 if missing/null
        std::string name = standing.value("name", "unknown");
        leaderboard.emplace(score,name);
    }
    std::cout << "\n--- LEADERBOARD ---\n";
    while(!leaderboard.empty())
    {
        std::cout << leaderboard.top().second << ':' << leaderboard.top().first << std::endl;
        leaderboard.pop();
    }
}
