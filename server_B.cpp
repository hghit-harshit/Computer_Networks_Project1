...
void handle_client(int client_socket,int socket_llm)
{
    ...
    while(question_no < 10)
    {
        ...
        if (std::chrono::duration_cast<std::chrono::seconds>(now - last_keepalive).count() >= 10) 
        {
            std::cout << "Disconnecting Client as it is not responding to keep alive messages\n";
            close(client_socket);
            
        }
        else if (std::chrono::duration_cast<std::chrono::seconds>(now - last_keepalive).count() >= 5) 
        {
